...
document.getElementById('colorPicker').addEventListener('input', (event) => {
  ...
  const color = new cv.Scalar(selectedColor.b, selectedColor.g, selectedColor.r, 255); // OpenCV uses BGR format
  ...
  for (let i = 0; i < src.rows; i++) {
    for (let j = 0; j < src.cols; j++) {
      if (mask.ucharPtr(i, j)[0] !== 255) { // If pixel is NOT part of the wall
